...
save_weights_dir = os.path.join(os.path.dirname(__file__), 'weights')
model_path = {
    'body':
...
def openposeDrawBodyPose(canvas, candidate, subset):
    stickwidth = 4
...
# Written by Lvmin
def openposeFaceDetect(candidate, subset, oriImg):
    ...
    for person in subset.astype(int):
        ...
        if not (has_left_eye or has_right_eye or has_left_ear
                or has_right_ear):
            continue

        head, left_eye, right_eye, left_ear, right_ear = person[[
            0, 14, 15, 16, 17
        ]]
...
def openposeDrawFacePose(canvas, all_lmks, eps=1e-3):
    H, W, C = canvas.shape
...
def smart_resize(x, s):
    ...
    if Co == 3 or Co == 1:
        ...
        return np.stack([smart_resize(x[:, :, i], s) for i in range(Co)],
                        axis=2)
